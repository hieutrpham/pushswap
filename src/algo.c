...
/*@brief: prepare stack a so that the element with the least moves are pushed first
 */
void prep_stack_a(t_stack *stack_a, i_list **list, int index_top, int index_bot)
{
	...
	if ((num_ra <= num_rra && num_ra > 0 && num_rra > 0) || (num_ra > 0 && num_rra < 0))
		do_rotate(stack_a, list, ra, num_ra);
	else if ((num_ra > num_rra && num_ra > 0 && num_rra > 0) || (num_ra < 0 && num_rra > 0))
...
/*@brief: core algorithm that insert elements from stack a to stack b based on a presorted array
 */
void insert_sort(t_stack *stack_a, t_stack *stack_b, i_list **list)
{
	...
	while (i < stack_a->chunk_size)
	{
		...
		// ft_printf("index bot: %d, start: %d, end: %d\n", index_bot, start_index, end_index);
